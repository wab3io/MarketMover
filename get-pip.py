...
async def on_ready():
    ...
    players["test_user"] = {"points": 50, "name": "Test"}  # Test entry
    print(f"Before save, players: {players}")  # Debug
    save_players(players)  # Test save
    print("Starting post_assets...")
    await post_assets()  # Manually trigger the three posts
    print("Post_assets completed")
    await asyncio.sleep(23400)  # Wait 6 hours 30 minutes for testing (simulate market day)
    print("Starting check_results...")
    await check_results()  # Manually trigger results
    print("Check_results completed")
    await asyncio.sleep(5)  # Extra delay to ensure save
    ...
    for category in ["crypto", "stock", "forex"]:
        ...
        for user_id, user_bets in bets.items():
            if category in user_bets:
                ...
                # Enforce 6-hour 30-minute window (23,400 seconds)
